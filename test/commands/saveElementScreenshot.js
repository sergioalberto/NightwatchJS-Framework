...
const _easyimage = require('easyimage');

const _easyimage2 = _interopRequireDefault(_easyimage);
 ...
 */
const SaveElementScreenshotAction = (function (_events$EventEmitter) {
  ...
  _createClass(SaveElementScreenshotAction, [{
    ...
    value: function command(elementSelector, fileName, defaultMessage) {
      ...
      this.api.getElementSize(elementSelector, sizeResult => _this2.api.getLocation(elementSelector, locationResult => _this2.api.saveScreenshot(fileName, () => {
    ...
    value: function crop(size, location) {
      ...
      return _easyimage2.default.crop({
        ...
        let message = `Saved screenshot for <${_this3.elementSelector}> to ${_this3.fileName}`;
      ...
      }, err => _this3.emit('error', `SaveElementScreenshotAction: could not save screenshot (error is ${err})`));
    },
  }]);
