...
const WaitForJqueryAjaxRequest = (function (_events$EventEmitter) {
  ...
  function WaitForJqueryAjaxRequest() {
    ...
    const _this = _possibleConstructorReturn(this, (WaitForJqueryAjaxRequest.__proto__ || Object.getPrototypeOf(WaitForJqueryAjaxRequest)).call(this));
  ...
  _createClass(WaitForJqueryAjaxRequest, [{
    ...
    value: function check(callback, maxTimeInMilliseconds) {
      ...
      const executeFunc = function executeFunc(selector) {
      ...
      return this.api.execute(executeFunc, [], (result) => {
        ...
        if (result.status === 0 && result.value === 0) {
          ...
          return setTimeout(() => _this3.check(callback, maxTimeInMilliseconds), _this3.timeoutRetryInMilliseconds);
        }
